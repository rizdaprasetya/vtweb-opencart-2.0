...
class ModelPaymentVeritransBin extends Model {
  
  public function getMethod($address, $total) {
		
		$this->load->language('payment/veritransbin');

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('veritransbin_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

		if ($this->config->get('veritransbin_total') > 0 && $this->config->get('veritransbin_total') > $total) {
			$status = false;
		} elseif (!$this->config->get('veritransbin_geo_zone_id')) {
			$status = true;
      ...
      $method_data = array(
        'code'       => 'veritransbin',
        'title'      => $this->config->get('veritransbin_display_name'),
		'sort_order' => $this->config->get('veritransbin_sort_order'),
		'terms' 	 => ''
      );
    }
